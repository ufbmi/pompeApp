...
open class Piechart: UIControl {
 let userDefaults = UserDefaults.standard
    ...
    open var info: String = "info" {
        didSet {
            infoLabel.text = info
            if infoLabel.text! == "3/9" {
                infoLabel.textColor = UIColor.clear
            } else {
    ...
    open override func draw(_ rect: CGRect) {
        ...
        let center = CGPoint(x: bounds.width / 2, y: screenHeight/9 )
        ...
        for (index, slice) in slices.enumerated() {
            ...
            path.move(to: center)
            path.addArc(withCenter: center, radius: screenHeight/9, startAngle: startAngle, endAngle: endAngle, clockwise: true)
